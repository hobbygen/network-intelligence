...
The separate **Alert history** page lists up to the last 200 stored alerts (newest first), with filters for process name, severity and direction, for browsing further back than the Dashboard's capped 50-row card. Filtering only changes what's shown; it never alters stored evidence. Use Refresh to pick up alerts fired since the page was last loaded.

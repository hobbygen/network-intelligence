...
## Accessibility pass (2026-09-19, ADR-019)

- `dotnet build NetworkIntelligence.slnx`: passed, zero warnings/errors. `dotnet test tests/NetworkIntelligence.Domain.Tests`: **138/138 passed** (unaffected — UI-only change).
- Computed WCAG relative luminance for the Dashboard's `#18BDAA`/`#6996F0` Download/Upload figures against each theme's card background: Light theme ~2.3:1/~2.8:1 (fails the 3:1 large-text minimum), Dark theme ~6.0:1/~4.9:1 (passes). Confirmed visually, not just computationally: rendered `--smoke-test --smoke-page Dashboard` against isolated databases seeded with `Theme=Light` and `Theme=Dark` settings — the Light-theme screenshot visibly showed both figures reading fainter than surrounding black text. After switching to new theme-aware `DownloadAccentBrush`/`UploadAccentBrush` resources (`#0C7367`/`#2F5FCC` for Light, ~5.3:1/~5.6:1; original values kept for Dark), re-rendered both themes: Light now reads clearly, Dark is unchanged pixel-for-pixel.
- Static audit of `MainWindow.xaml` for missing `AutomationProperties.Name`: found and fixed three gaps (`DiagnosticProgress`, the Dashboard traffic chart, `ApplicationSearch`) by comparing against sibling controls that already had the pattern applied elsewhere in the same file. Marked the About page's logo image decorative (`AccessibilityView="Raw"`) rather than adding a redundant name, since adjacent text already conveys the same information.
- Found and fixed three instances of ambiguous repeated per-row button labels ("Trust this app", "Snooze 1h", "Dismiss") that a screen-reader user navigating by control type couldn't distinguish between rows — added computed name properties to the existing `AppTrafficDisplay`/`AlertDisplay` wrapper records. One instance ("Untrust" in the Settings trusted-app list) was left as a known, documented gap since its `ItemsSource` is a plain `string[]` with no wrapper record to extend.
- Audited keyboard tab order: no `TabIndex` set anywhere in the file, so default declaration-order tab order applies and already matches visual reading order on every page. No changes needed.
- Not performed: native screen-reader software testing (Narrator, NVDA) exercising these fixes end to end, and WinAppDriver-style automated UI-accessibility testing — both require an interactive Windows session this environment doesn't have. The fixes above are the audit-level, statically/visually verifiable subset of accessibility work; a full pass (especially real screen-reader verification) remains outstanding.

## Remaining acceptance work

Controlled known-byte-traffic comparison for concurrent processes, real/physical adapters, UDP against a byte-exact reference, and low-rate/long-duration traffic; adapter disambiguation for per-process events; ETW session CPU/memory overhead benchmark; Wi-Fi SSID/signal/channel/security; internet/gateway reachability; adapter switching/sleep; MonitoringService code signing and dedicated least-privileged service account; automated unauthorized-client access tests; real false-positive-rate anomaly validation against live multi-day usage; stable application identity beyond process-name grouping (ADR-012 and ADR-018 each narrowed this gap — restart name-loss and mid-window pid reuse are fixed — but full executable-path identity, distinguishing two different applications that share a process name, remains open); network health score weights/curves validated against real user-perceived quality; Windows 10 and clean Windows 11 installs; broader speed-provider accuracy/availability testing; automated WinAppDriver-style UI automation and real screen-reader (Narrator/NVDA) verification (ADR-019 covered the static/visual subset: contrast, missing accessible names, ambiguous list-item labels — see above); long-duration/high-throughput benchmarks; signed release packaging.

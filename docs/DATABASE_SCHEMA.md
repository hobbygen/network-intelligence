# Implemented database schema — version 3

The migration is executed transactionally by `SqliteHistoryStore.InitializeAsync`, one version at a time (a database at version 1 gets only the version-2 and version-3 deltas applied, never a re-run of version 1). No destructive migration is automatic. Databases with a newer version than this build knows are rejected. SQLite uses WAL with a five-second busy timeout; a single asynchronous gate serializes this application's connections. Calls from the UI run on background tasks because SQLite's provider does not perform native asynchronous I/O.
...
History is capped by the configured 1–3650 day retention, applied identically to `ApplicationTrafficMinutes` and `AnomalyEvents`. Cleanup removes at most 2000 rows per table per pass, then reschedules until caught up. Live chart history is separately bounded to 90 points. Application executable paths, addresses and WLAN names are not stored. Adapter rows themselves retain last-seen metadata; history deletion clears them, `ApplicationTrafficMinutes` and `AnomalyEvents` included.

Stable application identities beyond process-name grouping, enhanced metrics and retained speed-test results remain planned and are not implied by schema 3. Back up the closed database before future migrations. `PRAGMA quick_check` is available from Diagnostics. Explicit history deletion is transactional and checkpoints WAL; it is not secure erasure.

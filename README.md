...
- Algorithmic per-application anomaly detection: learned baselines, independent download/upload evaluation, a learning period before any alert, sustained-duration + cooldown gating, trust exclusion, Dashboard alerts, a dedicated filterable Alert history page, and a Settings sensitivity control — detection thresholds are provisional, not validated against a real false-positive rate (see below).
